...
public class FileItemRepositoryCsvTests : IDisposable
{
    private readonly Faker<FileItem> _fileItemFaker;
    private readonly string _testDirectory;

    public FileItemRepositoryCsvTests()
    {
    ...
    public void AddRange_ShouldWriteAllTestCasesToFile()
    {
        ...
        var repository = new FileItemRepositoryCsv(options);

        var testCases = _fileItemFaker.Generate(5).ToArray();

        // Act
        repository.AddRange(testCases);

        // Assert
        var filePath = Path.Combine(_testDirectory, "fi.csv");
    ...
    public void AddRange_ShouldOverwriteExistingFile()
    {
        ...
        var firstBatch = _fileItemFaker.Generate(3).ToArray();
        var secondBatch = _fileItemFaker.Generate(5).ToArray();
    ...
    public void AddRange_ShouldCreateDirectoryIfNotExists()
    {
        ...
        var testCases = _fileItemFaker.Generate(1).ToArray();
